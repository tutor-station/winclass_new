...
function CalculateCommssion($iTransactionAmount=0)
{
  $arrReturn = array();
  if($iTransactionAmount>0)
  {
    $arrReturn['transaction_amount'] = $iTransactionAmount;
    $settings = Setting::select('id', 
                                'split_payment', 
                                'razorpay_linked_account', 
                                'razorpay_linked_account_name', 
                                'admin_commission', 
                                'razorpay_charges', 
                                'gst')
                                ->first();
    if($settings['split_payment']=="Y")
    {
      $iRazorPayCharges = round(($iTransactionAmount * $settings['razorpay_charges'])/100);
      $iRemainingAmount = $iTransactionAmount - $iRazorPayCharges;
      $iAdminCommision = round(($iRemainingAmount * $settings['admin_commission'])/100);
      $iGST = round(($iAdminCommision * $settings['gst'])/100);
      $iClientCommision = $iRemainingAmount - $iAdminCommision - $iGST;
      $iAdminCommision = $iAdminCommision + $iGST;
      $arrReturn['razorpay_charges'] = $iRazorPayCharges;
      $arrReturn['admin_commission'] = $iAdminCommision;
      $arrReturn['gst'] = $iGST;
      $arrReturn['client_commission'] = $iClientCommision;

      /*

      *"transfers" => array(
                                      array('account' => 'acc_KtnPFqe59o5h4v',
                                       'amount' => $iClass24Amount*100,
                                       'currency' => 'INR',
                                       'notes' => array('branch' => 'OTS ADDA PRIVATE LIMITED',
                                                      'name' => 'OTS ADDA PRIVATE LIMITED'
                                                      ), 
                                        'on_hold' => 0
                                      ),
                                      array(
                                          //'account' => 'acc_KtmseQLLzrfNmA',
                                          'account' => 'acc_LH9Cf20itIHY6w',
                                          'amount' => $iUnimediaAmount*100,
                                          'currency' => 'INR',
                                          'notes' => array('branch' => 'TutorAccess Technologies Pvt Ltd'
                                                          ,'name' => 'TutorAccess Technologies Pvt Ltd'
                                                          ), 
                                          'on_hold' => 0
                                      )
                                    )
      */
      $arrRazorPayJSON['transfers'] = array(
                                      array('account' => $settings['razorpay_linked_account'],
                                       'amount' => $iClientCommision*100,
                                       'currency' => 'INR',
                                       'notes' => array('branch' => $settings['razorpay_linked_account_branch'],
                                                      'name' => $settings['razorpay_linked_account_name']
                                                      ), 
                                        'on_hold' => 0
                                      )
                                    );
      $arrReturn['razorpay_split_payment_json'] = $arrRazorPayJSON;
    }
