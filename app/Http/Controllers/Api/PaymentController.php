...
class PaymentController extends Controller
{
    public function paystore(Request $request){
        ...
        if($request->pay_status == 1) {
            ...
            if ($request->cpn_amount != 0 || $request->cpn_amount != NULL)
            {
                
                $cpn_discount =  $request->cpn_amount;
            }
            else
            {
            ...
            $created_order = Order::where('razorpay_order_id', $request->razorpay_order_id)
                           ->update([
    ...
    public function purchasehistory(Request $request){
        ...
        $enroll = Order::where('user_id', $user->id)->where('transaction_id', '!=', '')->where('status', 1)->with('courses')->get();
    ...
    function createTransferRazorPay(Request $request)
    {
      $iPaymentId = "pay_Nbbmh6q8EM9toE";  
      $sConditions = "";
      if(!empty($iPaymentId))
      {
        $sConditions = " AND orders.transaction_id ='$iPaymentId' ";
      }
      $result = DB::select("SELECT * 
                            FROM orders 
                            WHERE orders.transaction_id !='' 
                            AND orders.`status` ='1'
                            AND orders.`client_commission` >0
                            AND orders.`transfer_status` ='P'
                            $sConditions"); 
      $arrOrder = json_decode(json_encode($result), true);
      //dd($arrOrder);
      if(count($arrOrder))
      {
        foreach($arrOrder as $iKey => $arrOrderDetail)
        {

          try
          {
            $pay_id = $arrOrderDetail['transaction_id'];
            $post_data = json_decode($arrOrderDetail['razorpay_split_payment_json']);
            //echo "<pre>";
            //print_r($post_data);
            ...
            //exit;
              ...
              $payment = $api->payment->fetch($pay_id)->transfer($post_data);
              echo "pre>";
              print_r($payment);
            }
            catch(\Exception $e)
           ...
           /* $sUrl = "https://api.razorpay.com/v1/payments/{$pay_id}/transfers";
    ...
    function checkRazorpayOrderStatus(Request $request)
    {
      $iOrderId = "";  
      $sConditions = "";
      if(!empty($iOrderId))
      {
        $sConditions = " AND orders.razorpay_order_id ='$iOrderId' ";
      }
      $result = DB::select("SELECT * 
                            FROM orders 
                            WHERE orders.razorpay_order_id !=''  
                            AND orders.is_processed = 'N'
                            $sConditions"); 
      $arrOrder = json_decode(json_encode($result), true);
      //dd($arrOrder);
      ...
      if(count($arrOrder))
      {
        foreach($arrOrder as $iKey => $arrOrderDetail)
        {
          ...
          try
          {
            $iRazorPayOrderId = $arrOrderDetail['razorpay_order_id'];
            $iOrderTableId = $arrOrderDetail['id'];
            $post_data = json_decode($arrOrderDetail['razorpay_split_payment_json']);
            //echo "<pre>";
            //print_r($post_data);
            $post_data = json_decode(json_encode($post_data), true);
            ...
            //exit;
              ...
              $arrPayments = $api->order->fetch($iRazorPayOrderId)->payments()->toArray();
              //echo "HERE 234";
              //echo "<pre>";
              //print_r($arrPayments);
              //exit;
              ...
              $bIsSuccessUpdated = false;
              if(isset($arrPayments['items']) && count($arrPayments['items'])>0){
              foreach ($arrPayments['items'] as $iKey => $arrData) { 
                $sStatus = strtolower($arrData['status']); 

                if(($sStatus=='captured' && $bIsSuccessUpdated===false) || ($sStatus=='failed' && $bIsSuccessUpdated===false) || ($sStatus=='refunded' && $bIsSuccessUpdated===false) ) {

                  // echo "HEEEEEE ";
                  $sRazorPayPaymentId = $arrData['id'];
                  $sRazorPayOrderId_Resp = $arrData['order_id'];
                  $payment_mode = strtolower($arrData['method']);
                  $response_code = $sStatus;                   
                  $transaction_date = date('Y-m-d H:i:s', $arrData['created_at']); 
                  $failure_message = $arrData['error_description'];              
                  $status_message = $arrData['error_reason'];              

                  $sResponseJson =  json_encode($arrPayments);
                  $transaction_status = $arrOrderDetail['payment_status'];
                  if($transaction_status=="P" ||  $transaction_status=="F" || $transaction_status=="S")
                  {

                    if($sStatus=='captured'){
                      $status_to_update = "S";
                      $bIsSuccessUpdated = true;
                      $message = 'Transaction has been done successfully and Updated In System !...';

                    }
                    else if($sStatus=='refunded'){
                      $status_to_update = "R";
                      $bIsSuccessUpdated = true;
                      $message = 'Transaction has been done successfully and Updated In System !...';
                    ...
                    {
                      $status_to_update = "F";
                      $message = 'Transaction has been Failed!... and  Updated In System ';
                    }
                    //echo "HERE => ";
                    $arrDetails = array( 
                      'payment_status'              => $status_to_update,
                      'payment_mode'            => $payment_mode,
                      'transaction_date'                  => $transaction_date,
                      'transaction_id'             => $sRazorPayPaymentId, 
                      'failure_message'             => $failure_message,
                      'status_message'            => $status_message, 
                      'enc_response'            =>  $sResponseJson,
                      'is_processed'             => "Y", 
                    );
                   //echo "<pre>";
                   //print_r($arrDetails);
                   //exit;
                    ...
                    Order::where('id',$iOrderTableId)->update($arrDetails); 
                    //show_message($message);
                  } 
                    
                   else if($transaction_status=="S")
                   { 
                    $message = 'Transaction has been done successfully and  Alraedy Updated In System!...';
                       // show_message($message);
                   }
            ...
            {
              //error_message("No Record Found For Given Order No");  
              //echo "HERE 1";
              $status_to_update="F";
                 $arrDetails = array(  
                  'payment_status'              => $status_to_update,  
                      'failure_message'             => "No Record Found in Razorpay", 
                      'is_processed'             => "Y", 
                    );
                 //echo "HERE 2";
                   //echo "<pre>";
    ...
    public function orderSave(Request $request){
        ...
        if (!$request->razorpay_order_id || $request->razorpay_order_id == "") {
            return response()->json(['Order Id is required.']);
        }
        ...
        $arrPaymentDetail = CalculateCommssion($request->payamount);
        ...
        $created_order = Order::create([
            ...
            'razorpay_create_order_request' => json_encode($request->razorpay_requestData),
            'razorpay_create_order_response' => $request->razorpay_responseData,
            'total_amount' => $request->payamount,

            'payment_method' => 'Razorpay',
            'payment_status' => 'P',
            'payment_source' => "APP",
            'admin_commission' => $arrPaymentDetail['admin_commission'],
            'client_commission' => $arrPaymentDetail['client_commission'],
            'gst' => $arrPaymentDetail['gst'],
            'razorpay_charges' => $arrPaymentDetail['razorpay_charges'],
            'razorpay_split_payment_json' => json_encode($arrPaymentDetail['razorpay_split_payment_json']),
            'created_at'  => \Carbon\Carbon::now()->toDateTimeString(),
            ]
        );
        ...
        //     $coursedata = Course::where('id', '=', $cart->course_id)->first();
        //     $arrPaymentDetail = CalculateCommssion($coursedata->discount_price);
        ...
        return response()->json('Order Save Successfull !', 200);
