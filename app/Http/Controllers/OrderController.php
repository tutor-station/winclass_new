...
class OrderController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (empty($User)) {
            Session::flash('delete', trans('flash.UserNotRegistered'));
        ...
        // }
         ...
         if (isset($request->course_id)) {
            ...
            $created_course = Order::create([
                ...
                'total_amount' => ($course->price - $course->discount_price),
                'payment_method' => 'Admin Enroll',
                'created_at'  => \Carbon\Carbon::now()->toDateTimeString(),
            ]);
    ...
    public function purchasehistory()
    {
        ...
        {
            $orders = Order::where('refunded', '0')->where('user_id', Auth::user()->id)->where('transaction_id', '!=', '')->get();
