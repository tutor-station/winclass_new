...
{
    public function orderstore($txn_id, $payment_method, $sale_id = NULL, $file = NULL, $payment_status = NULL, $auth_user_id=NULL, $payment_source=false, $razorpay_order_id)
    {


        if(Session::get('one_order_course') !== null && Session::get('one_order_user') !== null)
        {
            return $this->oneorder($txn_id, $payment_method, $sale_id, $file, $payment_status, $auth_user_id, $razorpay_order_id);
        }
        ...
        foreach($carts as $cart)
        {
            ...
            $created_order = Order::where('razorpay_order_id', $razorpay_order_id)
                       ->update([
                            'course_id' => $course_id,
                            'user_id' => Auth::User()->id,
                            'instructor_id' => $instructor_id,
            ...
            if($lastOrder){
                $lastOrder = Order::where('razorpay_order_id', $razorpay_order_id)->first();
            }
            ...
            if($cart->type == 0)
            {
                ...
                if($lastOrder){
                    ...
                    $url = route('view.order', $lastOrder->id);
    ...
    public function oneorder($txn_id, $payment_method, $sale_id = NULL, $file = NULL, $payment_status = NULL, $auth_user_id=NULL, $razorpay_order_id)
    {
        ...
        // }
            ...
            $coursedata = Course::where('id', '=', $course->id)->first();
