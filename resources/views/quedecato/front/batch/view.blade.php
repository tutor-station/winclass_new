...
<div class="container">
    <div class="row"> 
        ...
        <div class="courses-item mb-30 hover-zoomin">
            <div class="thumb fix">
                <a href="{{ route('batch.frontdetail',$datas->id) }}"><img src="{{ asset('images/batch/'.$datas->preview_image) }}" alt="contact-bg-an-01"></a>
            </div>
            <div class="courses-content">                                    
                <div class="view-user-img">
                    <a href="#" title="">
                        @if($datas->user['user_img'] !== NULL && $datas->user['user_img'] !== '')
                        <img src="{{ asset('images/user_img/'.$datas->user['user_img']) }}" class="img-fluid user-img-one" alt="">
                        @else
                        <img src="{{ Avatar::create($datas->title)->toBase64() }}" alt="img">
                        @endif
                    </a>
                 </div>                                
                <h3><a href="{{ route('batch.frontdetail',$datas->id) }}"> {{ $datas->title }}</a></h3>
                <p>{{substr(preg_replace("/\r\n|\r|\n/",'',strip_tags(html_entity_decode($datas->detail))), 0, 120)}}</p>
                <a href="{{ route('batch.frontdetail',$datas->id) }}" class="readmore">{{__('View More')}}<i class="fal fa-long-arrow-right"></i></a>
            </div>
            <div class="icon">
