        ...
        <form action="{{route('start.quiz.store',$topic->id) }}" method="POST" id="question-form">
          {{ csrf_field() }}
            <?php $count=1; ?>
                ...
                <div class="row">
                  <div class="col-lg-8">
                    <div class="jumbotron" id="quiz{{ $key+1 }}" >
                        <h4 style="color:black;">Q{{$count}}.&emsp;{{ $array['question'] }}</h4><br>
                        <input type="hidden" id="question_id[{{$count}}]" name="question_id[{{$count}}]" value="{{ $array['id'] }}">
                        
                        <input type="hidden" id="canswer->{{$count}}" name="canswer[{{$count}}]" value="{{ $array['answer'] }}">
                        
                        <label class="radio">
                           <input type="radio" class="q_answer" required name="answer[{{$count}}]" value="A"> {{ $array['a'] }}
                        </label><br><br>
                        <label class="radio">
                           <input type="radio" class="q_answer" required name="answer[{{$count}}]" value="B"> {{ $array['b'] }}
                        </label><br><br>
                        <label class="radio">
                           <input type="radio" class="q_answer" required name="answer[{{$count}}]" value="C"> {{ $array['c'] }}
                        </label><br><br>
                        <label class="radio">
                           <input type="radio" class="q_answer" required name="answer[{{$count}}]" value="D"> {{ $array['d'] }}
                        </label><br><br>
                         <br>
                      </div>
                    </div>
                    ...
                    <div class="col-lg-4">
                       <div class="quiz-tabs">
                            ...
                            <ul class="nav nav-tabs" id="myTab" role="tablist">
                              ...
                              @if(isset($array['question_video_link']))
                              <li class="nav-item">
                                <a class="nav-link active" id="home-tab" data-toggle="tab" href="#home" role="tab" aria-controls="home" aria-selected="true">{{ __('Video')}}</a>
                              </li>
                              @endif

                              @if(isset($array['question_img']))
                              <li class="nav-item">
                                <a class="nav-link" id="profile-tab" data-toggle="tab" href="#profile{{ $array['id'] }}" role="tab" aria-controls="profile" aria-selected="false">{{ __('Image')}}</a>
                              </li>
                              @endif
                            ...
                            <div class="tab-content" id="myTabContent">
                              ...
                              <div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">
                                ...
                                <div style="left: 0; width: 100%; height: 0; position: relative; padding-bottom: 95.25%;">
                                  <iframe src="{{ $array['question_video_link'].'?modestbranding=1' }}" style="border: 0; top: 0; left: 0; width: 100%; height: 100%; position: absolute;" allowfullscreen scrolling="no" allow="encrypted-media">
                              ...
                              <div class="tab-pane fade" id="profile{{ $array['id'] }}" role="tabpanel" aria-labelledby="profile-tab">
                                <img src="{{ asset('images/quiz/'.$array['question_img']) }}" class=" img-fluid" alt="">
                              </div>
              ...
              <button style="display: none;" id="prev" title="{{ __('Click to see previous question')}}" class="btn btn-md btn-primary mb-5" value="1"><< {{ __('Prev') }}</button>
              @if($que_count >1)
              <button title="{{ __('Click to see next question')}}" id="next" class="pull-right btn btn-md btn-primary mb-5" value="0">{{ __('Next') }} >></button>
              @else
              <button title="Finish the quiz" type="submit" class="btn btn-md btn-primary mb-5">{{ __('Finish') }}</button>
              @endif
